...
python3 run.py --garment_path "$CLOTH_PATH" --garment_type "$GARMENT_TYPE"
...
for IMAGE_PATH in "$IMAGES_FOLDER"/body*.png; do
    python run_ootd.py --model_path "$MODEL_PATH" --cloth_path "../../$CLOTH_PATH" --scale "$SCALE" --sample "$SAMPLE" --image_path "$IMAGE_PATH" ----model_type "$MODEL_TYPE" --category $CATEGORY
done
